...
submarine, so you turn them off for now. There are 100 octopuses arranged neatly in a 10 by 10 grid. Each octopus slowly
gains energy over time and flashes brightly for a moment when its energy is full. Although your lights are off, maybe
you could navigate through the cave without disturbing the octopuses if you could predict when the flashes of light will
happen. Each octopus has an energy level - your submarine can remotely measure the energy level of each octopus (your
...
bottom-right one has an energy level of 6, and so on. You can model the energy levels and flashes of light in steps.
...
func runIteration(oc *OctopusCavern) *OctopusCavern {
	...
	for i := 0; i < octopusIncrementGoroutines; i++ {
		go func(oc *OctopusCavern, outputs []RecursiveChan, rowTokens chan int, wg *sync.WaitGroup) {
			defer wg.Done()
...
func (oc OctopusCavern) getRowCount() int {
	return len(oc.grid)
}

func (oc OctopusCavern) getColumnCount() int {
	if len(oc.grid) == 0 {
		return 0
	}
...
func (oc *OctopusCavern) increment(i int, j int) int64 {
	v := atomic.AddInt64(&oc.grid[i][j], 1)
	if v == octopusFlashValue {
		atomic.AddInt64(&oc.flashes, 1)
	}
	return v
}
...
func makeRecursiveChan(capacity int) RecursiveChan {
	return RecursiveChan{
		c:       make(chan OctopusFlashEvent, capacity),
		counter: &sync.WaitGroup{},
	}
}

func (r RecursiveChan) add(o OctopusFlashEvent) {
	r.counter.Add(1)
	r.c <- o
}
